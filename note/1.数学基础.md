...
考虑不等式约束 $g(x) \leq 0$  。当 $g(x) < 0$  ，约束不起作用，可以直接通过 $\nabla f(x)=0$ 获得最优点 $x^*$ ，如果 $g(x^*)>=0$ 就说明 $g(x)$ 约束起作用了，说明 $x^*$ 在 $g(x)=0$ 边界上；当 $g(x) = 0$ ，与上述等式约束相似，不过 梯度 $\nabla f(x^*)$ 和 $\nabla g(x^*)$ 此时必相反（**即不能同时减少**），即 $\lambda >0$ 。拉格朗日函数的约束可以转化为：
...
对拉格朗日函数 $L(x,\lambda,\mu)$ , 如果固定了 $\lambda,\mu$:

$$
\begin{aligned}
\Gamma(\lambda,\mu)&=\inf_{x \in D}L(x,\lambda,\mu)\\
&=\inf_{x \in D}\left(f(x)+\sum_{i=1}^m\lambda_ih_i(x)+\sum_{j=1}^n\mu_jg_j(x)\right)\\
&\leq L(x^*,\lambda,\mu) \leq f(x^*)
\end{aligned}
$$

所以 $\Gamma(\lambda,\mu)$ 给出了 $f(x^*)$ 的下界，这个下界取决于 $\lambda,\mu$ 的取值。基于 $\lambda,\mu$ 去最大化这个下界以靠近 $p^*$:

$$
d^*=\max_{\lambda,\mu} \Gamma(\lambda,\mu) \\
\mu\geq0
$$

> 可以通过 $\Gamma$ 对 $x$ 求导并使其为0将 $x$ 用 $\lambda,\mu$ 表示。

如果  $d^* \leq p^*$ ，则为弱对偶性； $d^*=p^*$ 则为强对偶性，强对偶性时可通过 $L(x,\lambda,\mu)$ 分别对 $x,\lambda,\mu$ 求导并使其为0解决。 
