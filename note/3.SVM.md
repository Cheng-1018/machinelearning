...
$$
\begin{aligned}
L(\boldsymbol{w},b,\boldsymbol{\alpha})&=\frac{1}{2}\lVert\boldsymbol{w}\rVert^2+\sum_{i=1}^m \alpha_i(1-y_i(\boldsymbol{w}^T\boldsymbol{x_i}+b))  \\
KKT&\begin{cases}
\alpha_i &\geq 0 \\
...
\max _{\boldsymbol\alpha}L(\boldsymbol{w},b,\boldsymbol{\alpha})&=\frac{1}{2}\lVert\sum_{i=1}^m\alpha_iy_i\boldsymbol{x_i}\rVert^2+\sum_{i=1}^m\alpha_i-\sum_{i=1}^m\sum_{j=1}^m\alpha_i\alpha_jy_iy_j\boldsymbol{x_j}^T\boldsymbol{x_i}\\
...
$$
\begin{aligned}
\kappa(x_i,x_j)&=exp(-\frac{\lVert x_i-x_j \rVert^2}{2\sigma^2})&，\sigma>0,高斯核 \\
...
$$
\min_{\boldsymbol{w},b} \lVert\boldsymbol{w}\rVert^2 +C\sum_{i=1}^m \mathcal{l}_{loss}(y_if(x_i)-1)
$$
...
$$
\min_{\boldsymbol{w},b} \lVert\boldsymbol{w}\rVert^2 +C\sum_{i=1}^m \mathcal{l}_{loss}(f(x_i)-y_i)
$$
